package com.tencent.qqnt.kernelgpro.nativeinterface;

import com.tencent.mobileqq.qfix.redirect.IPatchRedirector;
import com.tencent.qqnt.kernel.nativeinterface.RDeliveryLogicEnvronment;

/* loaded from: classes.jar:com/tencent/qqnt/kernelgpro/nativeinterface/GProInteraction.class */
public final class GProInteraction {
    static IPatchRedirector $redirector_;
    public String interactiveUid;
    public long timestamp;
    public int type;

    public GProInteraction() {
        IPatchRedirector iPatchRedirector = $redirector_;
        if (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 1)) {
            this.interactiveUid = "";
        } else {
            iPatchRedirector.redirect((short) 1, this);
        }
    }

    public String getInteractiveUid() {
        IPatchRedirector iPatchRedirector = $redirector_;
        return (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 2)) ? this.interactiveUid : (String) iPatchRedirector.redirect((short) 2, this);
    }

    public long getTimestamp() {
        IPatchRedirector iPatchRedirector = $redirector_;
        return (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 4)) ? this.timestamp : ((Long) iPatchRedirector.redirect((short) 4, this)).longValue();
    }

    public int getType() {
        IPatchRedirector iPatchRedirector = $redirector_;
        return (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 3)) ? this.type : ((Integer) iPatchRedirector.redirect((short) 3, this)).intValue();
    }

    public String toString() {
        IPatchRedirector iPatchRedirector = $redirector_;
        if (iPatchRedirector != null && iPatchRedirector.hasPatch((short) 5)) {
            return (String) iPatchRedirector.redirect((short) 5, this);
        }
        return "GProInteraction{interactiveUid=" + this.interactiveUid + ",type=" + this.type + ",timestamp=" + this.timestamp + ",}";
    }
}
