package com.tencent.qqnt.kernelgpro.nativeinterface;

import com.tencent.mobileqq.qfix.redirect.IPatchRedirector;
import com.tencent.qqnt.kernel.nativeinterface.RDeliveryLogicEnvronment;

/* loaded from: classes.jar:com/tencent/qqnt/kernelgpro/nativeinterface/GProSendNoticeRsp.class */
public final class GProSendNoticeRsp {
    static IPatchRedirector $redirector_;
    public int code;
    public String notAllowReason;

    public GProSendNoticeRsp() {
        IPatchRedirector iPatchRedirector = $redirector_;
        if (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 1)) {
            this.notAllowReason = "";
        } else {
            iPatchRedirector.redirect((short) 1, this);
        }
    }

    public int getCode() {
        IPatchRedirector iPatchRedirector = $redirector_;
        return (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 2)) ? this.code : ((Integer) iPatchRedirector.redirect((short) 2, this)).intValue();
    }

    public String getNotAllowReason() {
        IPatchRedirector iPatchRedirector = $redirector_;
        return (iPatchRedirector == null || !iPatchRedirector.hasPatch((short) 3)) ? this.notAllowReason : (String) iPatchRedirector.redirect((short) 3, this);
    }

    public String toString() {
        IPatchRedirector iPatchRedirector = $redirector_;
        if (iPatchRedirector != null && iPatchRedirector.hasPatch((short) 4)) {
            return (String) iPatchRedirector.redirect((short) 4, this);
        }
        return "GProSendNoticeRsp{code=" + this.code + ",notAllowReason=" + this.notAllowReason + ",}";
    }
}
